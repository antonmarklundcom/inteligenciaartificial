...
ALTER TABLE `newsletter_subscribers` ADD CONSTRAINT `newsletter_subscribers_consent_record_id_consent_records_id_fk` FOREIGN KEY (`consent_record_id`) REFERENCES `consent_records`(`id`) ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE `assessment_answers` ADD CONSTRAINT `assessment_answers_assessment_question_id_assessment_questions_id_fk` FOREIGN KEY (`assessment_question_id`) REFERENCES `assessment_questions`(`id`) ON DELETE no action ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `assessment_question_options` ADD CONSTRAINT `assessment_question_options_assessment_question_id_assessment_questions_id_fk` FOREIGN KEY (`assessment_question_id`) REFERENCES `assessment_questions`(`id`) ON DELETE no action ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `assessment_questions` ADD CONSTRAINT `assessment_questions_assessment_version_id_assessment_versions_id_fk` FOREIGN KEY (`assessment_version_id`) REFERENCES `assessment_versions`(`id`) ON DELETE no action ON UPDATE no action;--> statement-breakpoint
